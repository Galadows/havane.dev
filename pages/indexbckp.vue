<template>
  <div class="h-screen w-screen" :class="{ 'overflow-hidden': !canShowcase }">
    ...
    >
      ...
      <div v-if="canShowcase" class="animate-appear absolute bottom-5 h-5 w-5">
...
export default {
  ...
  methods: {
    async typeWriter(word, ref, delay = 100) {
      ...
      this.chat = ''
      for (let index = 0; indexbckp < word.length; indexbckp++) {
        const letter = word[indexbckp]
  ...
  computed: {
    ...
    really() {
      let really = ''
      for (let index = 0; indexbckp < this.no; indexbckp++) {
        really += ' really'
      }
      return really
    },
